...
export const Page = () => {
  ...
  const [firstDate, setFirstDate] = useState<Date>(new Date(Date.now() - 2 * 24 * 60 * 60 * 1000))
  ...
  const fetchDataByDateRange = async () => {
    ...
    try {
      ...
      if (e.code === "ECONNABORTED") {
        ...
        setError(`Failed to load tokens. Server returned ${e.response.status}: ${e.response.statusText}`)
      } else {
  ...
  useEffect(() => {
    const filtered = tokens.filter((token) => token.currencyName.toLowerCase().includes(searchQuery.toLowerCase()))
              ...
              {filteredTokens.length > 0 ? (
                filteredTokens.map((token, index) => (
                  <TableRow key={token.currencyName}>
                    ...
                    <TableCell className="font-medium">{token.currencyName}</TableCell>
                    <TableCell className="font-medium">
                      <CustomTooltip content={token.contract}>
                        <span className="cursor-help">{shortenAddress(token.contract)}</span>
                      </CustomTooltip>
                    </TableCell>
                    <TableCell>{token.percentageChange?token.percentageChange.toString().slice(0, 5):"Нет значения"}</TableCell>
                    <TableCell>{token.valueChange?token.valueChange:"Нет значения"}</TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={6} className="text-center">
                    {searchQuery ? "Нет результатов поиска" : "Нет данных для выбранного периода"}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
